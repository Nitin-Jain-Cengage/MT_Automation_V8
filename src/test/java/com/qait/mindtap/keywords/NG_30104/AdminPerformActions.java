...
public class AdminPerformActions extends GetPage{

    String OrgURL = yml.getData("environment") + "/static/nb/ui/admin/orgs/orgs.html" ;
    String MasterURL = yml.getData("environment") + "/static/nb/ui/admin/masters/masters.html";
    int AJAX_WAIT = 30;
    TakeScreenshot takescreenshot;
	
	
	public AdminPerformActions(WebDriver driver) {
       super(driver, "AdminPerformActionPage");
	}

	public void verify_User_LoggedIn() {
		 Reporter.pass ("Completed LoginAsAdmin");
		 verifyPageTitleContains(yml.getData("neXtBooks.neXtBook1.pageTitle"));
	     isElementDisplayed("logo");
	     isElementDisplayed("loggedID");
	     Reporter.pass ("Completed verifyPageTitle");
	}
	...
	public void openMaster() {
        ...
        Reporter.pass ("Completed Admin Creates Master NeXtBook In Course Mode");
        
	}

	public void clickCreateMaster() {
          isElementDisplayed("admin_btn");
          element("createMaster").click();
		
	}

	public void enterMasterNameAndDescription(String title, String learningpathname,String description) {
            element("neXtBookName_txtField").sendKeys(title);
            element("neXtBookLearningPathName_txtField").clear();
            element("neXtBookLearningPathName_txtField").sendKeys(learningpathname);
            element("neXtBookDescription_txtField").sendKeys(description);
        }

	public void enterCoreTextISBN(String isbn) {
            element("coreTextISBN_txtField").clear();
            element("coreTextISBN_txtField").sendKeys(isbn);//.tolong.tostring
	}

	public void createUnpublishedMaster(String title) {
            element("createNeXtBookFormSubmit_btn").click();
